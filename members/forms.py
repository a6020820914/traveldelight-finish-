...
class DateWhereForm(forms.Form):
    #出發日期欄位
    firstDate = forms.DateField(widget=forms.DateInput(attrs={'class' : 'form-control', 'type':'date', 'id':"firstDate", 'value':datetime.today().date()}), required=True, label = '出發日期')
    #回程日期欄位
    lastDate = forms.DateField(widget=forms.DateInput(attrs={'class' : 'form-control', 'type':'date', 'value':datetime.today().date()-relativedelta(months=-2)}), required=False,label = '回程日期')
    #目的地欄位
    seleSite = forms.ModelChoiceField(queryset=Site.objects.all(), empty_label="目的地", label = '您想去的地方', required=False)
    #增加屬性或條件用
    seleSite.widget.attrs.update({'class':"form-select", 'id':"seleSite"})
    firstDate.widget.attrs.update({'min':datetime.today().date()})
